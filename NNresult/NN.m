...
% 資料資訊
%--------------------------------------------------------------------------
load('C:\Users\nick\Documents\MATLAB\Data_10m\Normal_Data_Feature');
run_index1=[1];

train_datau=H_NH_Train_Feature_Result;%值
train_datay=Train_datay;%符號
test_datau=H_NH_Test_Feature_Result;%值
test_datay=Test_datay;
...
for run_index=run_index1
    ...
    information.number_of_input=2;%輸入維度der大小
...
%             end
%         end
%         if ((sucessNH+sucessH)>sum)
            name=['brnn_',num2str(test_col),'_',num2str(sucessNH+sucessH),'_',num2str(sucessNH),'_',num2str(sucessH)];
            save(name,'brnn','train_datau','test_datay','test_datau','train_datay','final','Information');
